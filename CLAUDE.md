...
| `/flatten confirm` | `cmd_flatten` | Close ALL positions on the next cycle (forced full-exits, bypass anti-churn). Guarded: requires the literal `confirm` arg. Writes `trading:manual:flatten`; a ✅ ack is pushed once applied — **admin only** |
| `/close TICKER` | `cmd_close` | Force a full exit of one position on the next cycle (Redis `trading:manual:close`); ✅ ack pushed once applied — **admin only** |
...
### Upgrade subsystems (2026-07 roadmap — `feat/multi-agent-orchestrator`)
Layered on top of the Orchestrator; every piece is free, fail-open, and behind kill-switches.
- **Data-provider abstraction** (`src/data/`) — `MarketDataProvider` interface + `CompositeProvider` fail-over router (`get_provider()` from `DATA_PROVIDER_PRIORITY`). Providers: `yfinance` (default), `stooq` (free EOD cross-check), `edgar` (official XBRL fundamentals, no key), `finnhub` (quote/news/earnings, key in header), `alphavantage` (25/day fallback). `QuantEngine` OHLCV + live-price route through it. **Cross-validation** (`src/data/validation.py`): `consensus_price` (median-robust, flags disagreement > `DATA_CONSENSUS_TOLERANCE_PCT`), `cross_check_close` → Redis `data:quality:{ticker}` (only when `DATA_CROSSCHECK_PROVIDERS` set); `crosscheck_fundamentals` (yfinance vs EDGAR shares). Free Treasury 10Y-2Y yield-curve fallback for the macro gate: `src/trading/treasury.py`.
- **Infra agents** (`src/trading/infra_agents.py`, `INFRA_AGENTS_ENABLED`) — non-strategy, read-only, **tighten-only** cycle observers aggregated into an `InfraDirective` (block/scale buys, shrink budget; never widen). `DataValidationAgent` (reads `data:quality:*`), `SentimentAgent` (reads cached `sentiment:*`), `RiskOverseerAgent` (correlation `CORRELATION_MAX` 0.70 / portfolio-beta `PORTFOLIO_BETA_MAX` 1.20 / sector-cluster `SECTOR_CLUSTER_MAX` 0.30, pure math in `src/trading/risk_factors.py`). Run in `Orchestrator._run_infra_agents`; reports persisted to `trading:orchestrator:state`.
- **Shadow pipeline** (`src/trading/shadow.py`, `SHADOW_STRATEGY_NAMES`) — a walk-forward winner runs `SHADOW_MODE_DAYS` (30) in read-only paper mode (`trading:shadow:{name}`, `promotion_ready`) before promotion into `default_strategies()`.
- **Concept-drift monitor** (`src/trading/drift_monitor.py`) — pure CUSUM + hand-rolled two-sample KS (no scipy); `check_and_quarantine` writes `trading:quarantine:{name}` and the Orchestrator excludes quarantined strategies from active buys (signals still recorded → auto re-enable on recovery). Knobs: `DRIFT_KS_ALPHA`, `DRIFT_CUSUM_THRESHOLD`, `DRIFT_MIN_SAMPLES`.
- **Notifier** (`src/agents/notifier.py`) — instant vs batched-digest routing (`DIGEST_INTERVAL_MINUTES`), `build_rationale`, `trading:digest:pending`. `TradingAgent._notify` delegates to it.
- **Manual ops** (`src/trading/manual_ops.py`) — `/flatten` + `/close` write Redis flags; `Orchestrator._run_manual_ops` drains them at cycle top (forced full-exits via `validate_sell`, bypass anti-churn), writes an ack (`trading:manual:ack`) + pushes a ✅ confirmation.
- **Security** — `src/utils/redact.py` (structlog processor scrubbing secret URLs/tokens/values from every log event; wired in `configure_logging`); `src/mcp/auth.py` (`require_mcp_auth` X-MCP-Token guard on all `/tools/*` via `MCP_AUTH_TOKEN` + `host_allowed` strict SSRF allowlist). CI: gitleaks + pip-audit + CodeQL + dependabot; local `scripts/security_scan.sh`.
- **Resilience** — `src/utils/watchdog.py` (`Watchdog`, 4th `main.py` task: RAM/CPU/Redis health → `system:degraded` flag + Telegram alert, psutil-optional; `WATCHDOG_*`); `src/utils/preflight.py` (`run_preflight()` boot readiness self-check, redacted report, fail-open).

### Backtesting Framework (`src/backtest/` — offline, local, free)
Standalone process (never touches StockArena/Redis/Postgres; yfinance + local parquet cache only). Tests all 26 strategies — the live 8 plus the 18 experimental ones — and the full agent (simulated with the expanded 26-strategy set) on up to ~60 years of daily bars, and feeds the results back to the live allocator as cold-start weight seeds. The live agent itself keeps trading only `default_strategies()` until a strategy is explicitly promoted. **Data-quality gate**: `validate.py` (`validate_history`, run inside `load_history`) strips corrupt LEADING segments — frozen runs (>60 identical closes) and adjustment artifacts (>250% single-bar jumps) — before any indicator sees them (a real audit found HUBB's 1,861-bar 1977–84 flat run + 886% artifact jump feeding every prior backtest); genuine history and real crashes (2008, JAZZ +210% in 2009) are preserved. Re-runnable audit: `python -m src.backtest.validate`. **Survivorship bias** is documented, not "fixed": the pool is 100% current survivors, so all returns are upward-biased — the Hebrew report shows a prominent caveat banner.
...
**Upgrade subsystems (2026-07 roadmap — all free, fail-open):**
- Data providers: `DATA_PROVIDER_PRIORITY` (`yfinance`), `DATA_CROSSCHECK_PROVIDERS` (empty), `DATA_CONSENSUS_TOLERANCE_PCT` (0.02), `DATA_QUALITY_MAX_AGE_SECONDS` (3600), `SEC_EDGAR_USER_AGENT`, `FINNHUB_API_KEY` (empty → skipped)
- Infra agents: `INFRA_AGENTS_ENABLED` (true), `SENTIMENT_VETO_SCORE` (−0.5), `SENTIMENT_SCALE_SCORE` (−0.2), `CORRELATION_MAX` (0.70), `PORTFOLIO_BETA_MAX` (1.20), `SECTOR_CLUSTER_MAX` (0.30), `RISK_CORR_LOOKBACK_DAYS` (60)
- Shadow + drift: `SHADOW_MODE_DAYS` (30), `SHADOW_STRATEGY_NAMES` (empty), `DRIFT_KS_ALPHA` (0.05), `DRIFT_CUSUM_THRESHOLD` (0.15), `DRIFT_CUSUM_SLACK` (0.0), `DRIFT_MIN_SAMPLES` (20)
- Cockpit: `DIGEST_INTERVAL_MINUTES` (60)
- Security: `MCP_AUTH_TOKEN` (empty → MCP `/tools` unauthenticated; set in any networked deploy)
- Resilience: `WATCHDOG_ENABLED` (true), `WATCHDOG_MEM_PCT_MAX` (90), `WATCHDOG_CPU_PCT_MAX` (95), `WATCHDOG_CHECK_SECONDS` (30)
